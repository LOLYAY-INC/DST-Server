...
public record DiscordDetailsC2SPacket(
        long userId,
        long expectedPlayerNum
) implements Packet<ServerPostEncryptionPacketListener> {
    /**
     * The codec for the entire LoginSuccessS2CPacket.
     */
    public static final PacketCodec<DiscordDetailsC2SPacket> CODEC = PacketCodec.create(
            // Encoder
            (buf, packet) -> {
                buf.writeLong(packet.userId);
                buf.writeLong(packet.expectedPlayerNum);
            },
            // Decoder
            (buf) -> {
                return new DiscordDetailsC2SPacket(
                        buf.readLong(),
                        buf.readLong()
                );
            }
