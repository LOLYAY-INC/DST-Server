...
public record ForceReconnectC2SPacket(
    long guildId
) implements Packet<ServerPostEncryptionPacketListener> {
    /**
     * The codec for the entire LoginSuccessS2CPacket.
     */
    public static final PacketCodec<ForceReconnectC2SPacket> CODEC = PacketCodec.create(
            // Encoder
            (buf, packet) -> {
            ...
            // Decoder
            (buf) -> {
                return new ForceReconnectC2SPacket(
                        buf.readLong()
                );
            }
    );
