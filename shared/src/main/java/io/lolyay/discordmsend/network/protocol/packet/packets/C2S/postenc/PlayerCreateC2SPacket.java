...
public record PlayerCreateC2SPacket(
        long guildId
) implements Packet<ServerPostEncryptionPacketListener> {
    /**
     * The codec for the entire LoginSuccessS2CPacket.
     */
    public static final PacketCodec<PlayerCreateC2SPacket> CODEC = PacketCodec.create(
            // Encoder
            (buf, packet) -> {
            ...
            // Decoder
            (buf) -> {
                return new PlayerCreateC2SPacket(
                        buf.readLong()
                );
            }
    );
