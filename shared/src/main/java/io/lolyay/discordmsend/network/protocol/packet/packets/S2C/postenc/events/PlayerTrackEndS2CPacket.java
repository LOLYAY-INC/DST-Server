...
public record PlayerTrackEndS2CPacket(
        long guildId,
        int tackId,
        EndReason reason
) implements Packet<ClientPostEncryptionPacketListener>, EventPacket {
    /**
     * The codec for the entire LoginSuccessS2CPacket.
     */
    public static final PacketCodec<PlayerTrackEndS2CPacket> CODEC = PacketCodec.create(
            // Encoder
            (buf, packet) -> {
                buf.writeLong(packet.guildId());
                buf.writeVarInt(packet.tackId());
                buf.writeVarInt(packet.reason().ordinal());
            },
            // Decoder
            (buf) -> {
                return new PlayerTrackEndS2CPacket(buf.readLong(), buf.readVarInt(), EndReason.values()[buf.readVarInt()]);
            }
    );

    @Override
    public void apply(ClientPostEncryptionPacketListener listener) {
        listener.onPlayerTrackEnd(this);
    }
}